...
void ReadAccelerometerAndGyroscope(int* new_accelerometer_samples,
  ...
  *new_gyroscope_samples = 0;
  ...
  while (IMU.accelerationAvailable()) {
    const int index = acceleration_data_index % acceleration_data_length;
    acceleration_data_index += 3;

    float* data = &acceleration_data[index];
    if (!IMU.readAcceleration(data[0], data[1], data[2])) {
